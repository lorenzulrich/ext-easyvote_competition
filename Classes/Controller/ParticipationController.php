...
class ParticipationController extends AbstractController {
	...
	public function editAction(\Visol\EasyvoteCompetition\Domain\Model\Competition $competition = NULL, \Visol\EasyvoteCompetition\Domain\Model\Participation $participation = NULL) {
		...
		if ($communityUser = $this->getLoggedInUser()) {
			...
			$existingParticipation = $this->participationRepository->findOneByCompetitionAndCommunityUser($competition, $communityUser);
	...
	public function publishAction(\Visol\EasyvoteCompetition\Domain\Model\Participation $participation) {
		if ($communityUser = $this->getLoggedInUser()) {
			if ($communityUser === $participation->getCommunityUser()) {
				...
				$this->persistenceManager->persistAll();
				$this->participationRepository->updateCachedVotesAndCachedRankForCompetition($participation->getCompetition());
				$this->redirect('show', 'Competition', $this->request->getControllerExtensionName(), array('competition' => $participation->getCompetition(), 'participation' => $participation, 'openSharer' => 1, 'no_cache' => 1), $this->settings['competitionHomePid']);
			} else {
				// @todo access denied
			}
		} else {
	...
	public function deleteAction(\Visol\EasyvoteCompetition\Domain\Model\Participation $participation) {
		if ($communityUser = $this->getLoggedInUser()) {
			if ($communityUser === $participation->getCommunityUser()) {
				...
				$this->persistenceManager->persistAll();
				$this->participationRepository->updateCachedVotesAndCachedRankForCompetition($participation->getCompetition());
				$this->redirect('show', 'Competition', $this->request->getControllerExtensionName(), array('competition' => $competition), $this->settings['competitionHomePid']);
			} else {
				// @todo access denied
			}
		} else {
